...
function getNodeImagePath(nodeType){
	if(nodeType == 'Adult'){
		return 'assets/adult.svg';
	}
	else if(nodeType == 'Child'){
		return 'assets/child.svg';
	}
	else if(nodeType == 'Old'){
		return 'assets/old.svg';
	}
}
				...
				.selectAll("image")
					...
					.enter().append("image")
						...
						.attr("xlink:href", function(d){
							return getNodeImagePath(d.type);
						})
...
function infectNode(node){
	...
	if(Math.random() <= node.beta){
		...
		var svgNode = d3.select("svg")
	                        .selectAll('g.nodes')
	                            .selectAll('image[id="'+node.nodeId+'"]')
	                            .attr('xlink:href','assets/infected_child.svg');
	   	console.log(svgNode);
	}
	updateStats();
	return node;
...
function healNode(node){
	...
	if(Math.random() <= node.gamma){
		...
		var svgNode = d3.select("svg")
						.selectAll("g.nodes")
							.selectAll('image[id="'+node.nodeId+']')
							.attr('xlink:href','assets/resistant_child.svg');
	}
	else{
